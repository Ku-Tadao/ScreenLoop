...
namespace ScreenLoop.Backend.Core.Models
{
    internal class Settings
    {
        ...
        private string _contentFolder = Shared.PathUtils.Normalize(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyVideos), "ScreenLoop"));
        private string _cacheFolder = Shared.PathUtils.Normalize(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ScreenLoop"));
        ...
        private RecordingMode _recordingMode = RecordingMode.Buffer;
        ...
        private bool _showGameBackground = false;
        ...
        private List<MenuItemPreference> _menuItems = KnownMenuItemIds
            ...
            .ToList();
        private string _defaultMenuItem = "Replay Buffer";
        ...
        public int StorageLimit
        {
            ...
            set
            {
                var (_, totalGb) = Windows.Storage.StorageService.GetContentDriveSpaceGb();
                int maxLimit = Math.Max(1, (int)Math.Floor(totalGb > 0 ? totalGb : value));
                _storageLimit = Math.Clamp(value, 1, maxLimit);
            }
        }
        ...
        [JsonPropertyName("alwaysRecord")]
        public bool AlwaysRecord
        {
            get => _alwaysRecord;
            set
            {
                if (_alwaysRecord != value)
                {
                    _alwaysRecord = value;
                }
            }
        ...
        public string DefaultMenuItem
        {
            ...
            set
            {
                ...
                {
                    _defaultMenuItem = "Replay Buffer";
                    return;
                }
    ...
    internal class Recording
    {
        ...
        {
            get => _filePath;
            set => _filePath = ScreenLoop.Backend.Shared.PathUtils.NormalizeOrNull(value);
    ...
    public class Content
    {
        ...
        {
            get => _filePath;
            set => _filePath = ScreenLoop.Backend.Shared.PathUtils.Normalize(value ?? string.Empty);
        }
    ...
    public class GameIntegrations
    {
        ...
        public GameIntegrationSettings CounterStrike2 { get; set; } = new GameIntegrationSettings(false);

        [JsonPropertyName("leagueOfLegends")]
        public GameIntegrationSettings LeagueOfLegends { get; set; } = new GameIntegrationSettings(false);

        [JsonPropertyName("pubg")]
        public GameIntegrationSettings Pubg { get; set; } = new GameIntegrationSettings(false);
