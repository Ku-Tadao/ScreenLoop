...
namespace ScreenLoop.Backend.Media
{
    internal class ContentService
    {
        ...
        public static async Task CreateMetadataFile(string filePath, Content.ContentType type, string game, List<Bookmark>? bookmarks = null, string? title = null, DateTime? createdAt = null, int? igdbId = null, bool isImported = false, List<string>? audioTrackNames = null, bool isFavorite = false)
        {
            bookmarks ??= [];
        ...
        public static async Task ToggleFavorite(JsonElement message)
        {
            if (!message.TryGetProperty("FileName", out JsonElement fileNameElement) ||
                ...
                !message.TryGetProperty("IsFavorite", out JsonElement isFavoriteElement))
            {
                Log.Error("ToggleFavorite missing FileName, ContentType, or IsFavorite.");
                return;
            }

            string fileName = fileNameElement.GetString() ?? string.Empty;
            string contentTypeStr = contentTypeElement.GetString() ?? string.Empty;

            if (!Enum.TryParse(contentTypeStr, true, out Content.ContentType contentType))
            {
                Log.Error($"Invalid ContentType provided for favorite toggle: {contentTypeStr}");
                return;
            }

            string metadataFilePath = PathUtils.Combine(FolderNames.GetMetadataFolderPath(contentType), $"{fileName}.json");
            var updated = await UpdateMetadataFile(metadataFilePath, c => c.IsFavorite = isFavoriteElement.GetBoolean());
            if (updated == null)
            {
                return;
            }

            var contentItem = AppState.Instance.Content.FirstOrDefault(c => c.FileName == fileName && c.Type == contentType);
            if (contentItem != null)
            {
                contentItem.IsFavorite = updated.IsFavorite;
            }

            await SettingsService.LoadContentFromFolderIntoState(true);
            Log.Information($"Favorite set to {updated.IsFavorite} for {fileName}");
        }
