...
internal static class MigrationService
{
    ...
    private static string appDataDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ScreenLoop");
    ...
    private static void Apply_0007_RenameVideoFolders()
    {
        try
        {
            ...
            // Check BOTH old location (.metadata in content folder) AND new location (AppData/ScreenLoop/metadata)
            int updatedCount = 0;
    ...
    // .metadata, .thumbnails, .waveforms -> AppData/Roaming/ScreenLoop/metadata, thumbnails, waveforms
