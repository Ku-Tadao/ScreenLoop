...
export function useAudioTracks(
  ...
  const applyMuting = useCallback(() => {
    ...
    for (const td of trackDataRef.current.values()) {
      let muted: boolean;
      if (solo !== null) {
        muted = td.ScreenLoopIndex !== solo;
      } else {
        muted = effectiveMuted.has(td.ScreenLoopIndex);
      }
      const vol = effectiveVolumes[td.ScreenLoopIndex] ?? 1;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      stopAllSources();
      vid.muted = localStorage.getItem('ScreenLoop-muted') === 'true';
    };
  ...
  const toggleTrackMute = useCallback((index: number) => {
    setMutedTracks((prev) => {
      ...
      if (index === 0) {
        ...
        for (const td of trackDataRef.current.values()) {
          if (td.ScreenLoopIndex !== 0) next.add(td.ScreenLoopIndex);
        }
        return next;
      }
      const next = new Set(prev);
