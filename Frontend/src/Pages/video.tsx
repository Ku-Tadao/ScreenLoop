...
export default function VideoComponent({ video }: { video: Content }) {
  ...
  const [isMuted, setIsMuted] = useState(() => {
    ...
    return localStorage.getItem('ScreenLoop-muted') === 'true';
  });
  const [playbackRate, setPlaybackRate] = useState(() => {
    const saved = localStorage.getItem('ScreenLoop-playbackRate');
  ...
  useEffect(() => {
    ...
    const onVolumeChange = () => {
      if (vid) {
        ...
        localStorage.setItem('ScreenLoop-volume', vid.volume.toString());
        localStorage.setItem('ScreenLoop-muted', vid.muted.toString());
      }
    };
    ...
    const onRateChange = () => {
      if (vid) {
        ...
        setPlaybackRate(r);
        localStorage.setItem('ScreenLoop-playbackRate', r.toString());
      }
    };
  ...
  const setPlayerVolume = (vol: number) => {
    ...
    setVolume(target);
    localStorage.setItem('ScreenLoop-volume', target.toString());
  ...
  const toggleMute = () => {
    if (videoRef.current) {
      if (audioTracks.isMultiTrack) {
        ...
        setIsMuted(newMuted);
        localStorage.setItem('ScreenLoop-muted', newMuted.toString());
        return;
      }
      ...
      <div className="flex flex-col flex-1 w-full h-full p-4 pb-2 overflow-hidden lg:w-3/4">
        ...
        >
          ...
          <div
            ...
            // Slid off-screen rather than unmounted, so keep its controls out of the tab order.
