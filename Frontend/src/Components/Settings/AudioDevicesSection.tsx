...
export default function AudioDevicesSection({
  ...
  useEffect(() => {
    if (settings.audioOutputMode !== 'All') {
      updateSettings({ audioOutputMode: 'All' });
    }
  }, [settings.audioOutputMode, updateSettings]);
  ...
  useEffect(() => {
    const storedSig = localStorage.getItem('screenloop.trackLimitWarnDismissedSig');
            ...
            <div className="py-2 flex items-center w-full">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  localStorage.setItem('screenloop.trackLimitWarnDismissedSig', selectionSig);
                }}
              >
                <X className="h-4 w-4" />
              </Button>
