...
## Repo Layout
- `ScreenLoop.sln` — solution root
- `Backend/` — app services, models, utils
- `Frontend/` — React + Vite app (TypeScript, Tailwind, DaisyUI)

## First-Time Setup
1. Clone the repo
   - `git clone https://github.com/Ku-Tadao/ScreenLoop.git && cd ScreenLoop`
...
- From the repo root:
  - `dotnet run --project ScreenLoop.csproj`
- Notes:
  - In Debug mode the app expects the frontend on `http://localhost:2882`.
  - If Bun is installed, the backend attempts to auto-run `bun run dev` in `Frontend/` if nothing is listening on 2882.
